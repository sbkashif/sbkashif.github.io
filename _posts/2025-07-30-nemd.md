...
1. **Detailed balancen**: There are net flows of particles or energy, so the forward and reverse processes do not balance out.
$$\sum_j W_{ij} P_i \neq \sum_j W_{ji} P_j$$
...
## Conclusion: Effective equilibrium in non-equilibrium steady states

Molecular dynamics simulations, while originally designed for equilibrium systems, can effectively model non-equilibrium steady states by leveraging the same core principles of ergodicity, Liouville’s theorem, and chaotic dynamics. The key is recognizing that while the system may not adhere to detailed balance or Boltzmann statistics, it can still reach a stationary distribution that allows for reliable predictions of macroscopic properties.